...
const main = async () => {
  program
    ...
    .requiredOption("--blockNumber <string>", "Start block of synching")
    .requiredOption(
      "--network <string>",
  ...
  let config = { ERC1820Registry: _contractAddress, ERC1820RegistryBlock: _blockNumber, network: _network };

  writeFile(path.resolve(__dirname, `../config/${_network}-${_contractAddress}.json`) ,JSON.stringify(config, null, 2))
